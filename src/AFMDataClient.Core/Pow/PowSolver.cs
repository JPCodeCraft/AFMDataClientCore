...
public partial class PowSolver : IDisposable
{
    ...
    internal string ProcessPow(PowRequest pow, CancellationToken cancellationToken = default)
    {
        ...
        {
            return ProcessPowBatch(inputBuffer, difficulty, cancellationToken);
        }

        if (UseBatchHashing && PowSha256Batch128.IsSupported
            && inputBuffer.Length <= PowSha256Batch128.MaxInputLength && difficulty.FirstHashByte >= 0)
        {
            return ProcessPowBatch128(inputBuffer, difficulty, cancellationToken);
        }
        ...
        while (true)
        {
            if ((ctr & 255) == 0) cancellationToken.ThrowIfCancellationRequested();
    ...
    private string ProcessPowBatch(byte[] inputBuffer, PowDifficulty difficulty, CancellationToken cancellationToken)
    {
    ...
    private string ProcessPowBatch128(byte[] inputBuffer, PowDifficulty difficulty, CancellationToken cancellationToken)
    {
        var batch = new PowSha256Batch128(inputBuffer);
        Span<Vector128<uint>> digest = stackalloc Vector128<uint>[8];
        Span<byte> hash = stackalloc byte[32];
        Span<byte> counterSpan = inputBuffer.AsSpan(4, 16);
        var firstWord = Vector128.Create(difficulty.FirstHashWord);
        var firstWordMask = Vector128.Create(difficulty.FirstHashWordMask);
        ulong counter = _counter;
        while (true)
        {
            cancellationToken.ThrowIfCancellationRequested();
            batch.Hash(counter, digest);
            uint candidates = Vector128.Equals(digest[0] & firstWordMask, firstWord)
                .ExtractMostSignificantBits();
