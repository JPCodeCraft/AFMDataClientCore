...
public sealed class PortfolioUploadService : IDisposable
{
    ...
    private readonly AsyncLocal<string?> _expectedAccount = new();
    private readonly SemaphoreSlim _importGate = new(1, 1);
    private CachedEntitlementDenial? _masterpieceDenial;

    public PortfolioUploadService(ClientCore core)
    {
        _uploads = core.Uploads;
        _authService = core.Auth;
    }
    ...
    {
        _expectedAccount.Value = _authService.AccountId;
        ...
        foreach (var group in groupedRequests)
        {
            ...
            try
            {
                ...
                var savedPosition = await SendWithUnauthorizedRecoveryAsync(
                    () => SendPortfolioAsync(HttpMethod.Post, position, cancellationToken),
    ...
    {
        var account = _expectedAccount.Value ?? _authService.AccountId;
        return !string.IsNullOrWhiteSpace(account) && account == _authService.AccountId
            && await _authService.GetTokenAsync(account, false, cancellationToken).ConfigureAwait(false) is not null;
    }

    private async Task<HttpResponseMessage> SendPortfolioAsync(HttpMethod method, object? payload, CancellationToken token)
    {
        var account = _expectedAccount.Value;
        if (string.IsNullOrWhiteSpace(account) || account != _authService.AccountId)
            throw new PortfolioUploadException("Your AFM account changed. Retry the operation after signing in.");
        return await _uploads.SendAfmAsync(method, "portfolio/positions", payload, account, token, backend: true).ConfigureAwait(false)
    ...
    private async Task<List<PortfolioPositionDto>> LoadPortfolioPositionsAsync(CancellationToken cancellationToken)
    {
        return await SendWithUnauthorizedRecoveryAsync(
            () => SendPortfolioAsync(HttpMethod.Get, null, cancellationToken),
    ...
    private static Task<HttpResponseMessage> SendWithUnauthorizedRecoveryAsync(Func<Task<HttpResponseMessage>> sendAsync)
        => sendAsync(); // Shared authenticated transport already retries once with the same account.
    private bool HasCachedMasterpieceDenial()
    {
        ...
        return denial is not null
            ...
            && string.Equals(denial.UserId, _expectedAccount.Value, StringComparison.Ordinal);
    }

    private void CacheMasterpieceDenial()
    {
        if (_expectedAccount.Value is not { Length: > 0 } userId)
        {
            return;
        }
