...
namespace Albion.Network
{
    internal sealed class AlbionParser : PhotonParser, IPhotonReceiver
    {
        ...
        public PacketReceiveResult Receive(AFMDataClient.Core.CapturedDatagram datagram)
        {
            lock (contextGate)
            {
                currentDatagram = datagram;
                try { return ReceivePacketDetailed(datagram.Payload); }
        ...
        private void NotifyObserved(object packet)
        {
            ...
            {
                try { observer(packet); }
                catch (Exception ex) { Log.Debug(ex, "Packet diagnostic observer failed"); }
            }
        }
        ...
        protected override void OnEvent(byte Code, Dictionary<byte, object> Parameters)
        {
            ...
            NotifyObserved(eventPacket);
            handlers.HandleAsync(eventPacket).GetAwaiter().GetResult();
        }
        ...
        protected override void OnRequest(byte OperationCode, Dictionary<byte, object> Parameters)
        {
            ...
            NotifyObserved(requestPacket);
            handlers.HandleAsync(requestPacket).GetAwaiter().GetResult();
        }
        ...
        protected override void OnResponse(byte OperationCode, short ReturnCode, string DebugMessage, Dictionary<byte, object> Parameters)
        {
            ...
            NotifyObserved(responsePacket);
            handlers.HandleAsync(responsePacket).GetAwaiter().GetResult();
        }
