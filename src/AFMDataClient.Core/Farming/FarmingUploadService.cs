...
public sealed class FarmingUploadService : IDisposable
{
    ...
    private readonly CoreSettings _settingsManager;
    private readonly IUploadAuthSession _authService;
    public event Action<UploadStatus, Guid, FarmingUploadSummary>? UploadCompleted;
    ...
    internal FarmingUploadService(CoreSettings settingsManager, IUploadAuthSession authService, HttpClient httpClient)
    {
        _settingsManager = settingsManager;
        _authService = authService;
        _httpClient = httpClient;
        _sessionAccountId = _authService.AccountId;
        _authService.AccountChanged += OnFirebaseUserChanged;
    ...
    private bool Enqueue<T>(string accountId, T value) where T : FarmingContext
    {
        ...
        if (Volatile.Read(ref _disposed) != 0 || !_settingsManager.Options.IslandTracking
    ...
    private void QueueUpload()
    {
        if (Volatile.Read(ref _disposed) == 0 && _settingsManager.Options.IslandTracking)
        {
            _queue.Writer.TryWrite(new QueueMessage(_authService.AccountId, null, true));
        }
    }

    private void OnFirebaseUserChanged()
    {
        UpdateUploadSession(_authService.AccountId);
    }

    private void OnSettingsChanged()
    {
        if (_settingsManager.Previous.IslandTracking == _settingsManager.Options.IslandTracking) return;

        UpdateUploadSession(_authService.AccountId, reset: true);
    ...
    private async Task ProcessQueueAsync()
    {
        while (await _queue.Reader.WaitToReadAsync().ConfigureAwait(false))
        {
            ...
            if (uploadRequested && Volatile.Read(ref _disposed) == 0
                ...
                && _authService.AccountId is { Length: > 0 } accountId
    ...
    private async Task PersistAsync(AccountOutbox outbox)
    {
    ...
    private async Task UploadBatchAsync(AccountOutbox outbox)
    {
        ...
        try
        {
            if (await _authService.GetTokenAsync(outbox.AccountId, false, cancellation.Token).ConfigureAwait(false) is null)
            {
    ...
    private async Task<HttpResponseMessage?> SendAsync(string accountId, byte[] payload, CancellationToken cancellationToken)
    {
        ...
        {
            var token = await _authService.GetTokenAsync(accountId, attempt != 0, cancellationToken).ConfigureAwait(false);
            if (!_settingsManager.Options.IslandTracking
                || !string.Equals(_authService.AccountId, accountId, StringComparison.Ordinal)
                || string.IsNullOrWhiteSpace(token))
            {
                return null;
            }

            var uri = new Uri(_httpClient.BaseAddress!, "dataclient/farming");
            ...
            response.Dispose();
            if (!string.Equals(accountId, _authService.AccountId, StringComparison.Ordinal))
            {
                return null;
            }
        }
    ...
    private string GetOutboxPath(string accountId)
    {
        var fileName = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(accountId))).ToLowerInvariant();
        return Path.Combine(_settingsManager.Options.StorageDirectory, "farming-outbox", fileName + ".json");
    }
