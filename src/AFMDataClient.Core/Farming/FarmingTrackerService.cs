...
public sealed class FarmingTrackerService : IDisposable
{
    ...
    private readonly ClientSession player;
    ...
    internal FarmingTrackerService(IUploadAuthSession auth, ClientSession player, FarmingUploadService uploader, CoreSettings settings)
    {
        this.auth = auth;
        this.player = player;
    ...
    {
        if (settings.Previous.IslandTracking == settings.Options.IslandTracking) return;
    ...
    private void OnAuthChanged()
    {
        lock (sync)
        {
            if (accountId == auth.AccountId) return;
            accountId = auth.AccountId;
            demolitions.Clear();
            islandMetadata.Clear();
            BeginTransition();
    ...
    {
        if (disposed || !settings.Options.IslandTracking || string.IsNullOrEmpty(accountId) || auth.AccountId != accountId) return false;
    ...
    private void ExpireDemolitions()
    {
        lock (sync)
        {
            if (disposed || !settings.Options.IslandTracking || string.IsNullOrEmpty(accountId)
                || accountId != auth.AccountId) return;
