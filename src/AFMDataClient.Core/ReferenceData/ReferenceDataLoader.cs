...
{
    private static readonly ReferenceDataOptions DefaultSettings = new();
    private readonly HttpClient httpClient;
    private readonly string cacheDirectory;
    private readonly SemaphoreSlim downloadSemaphore = new(3);
    private Func<ReferenceDataOptions> getReferenceDataOptions = () => DefaultSettings;

    public static ReferenceDataLoader Shared { get; private set; } = new(
        new HttpClient { Timeout = Timeout.InfiniteTimeSpan },
        Path.Combine(Path.GetTempPath(), "AFMDataClient", "reference-data"));

    public static void ConfigureShared(HttpClient client, string directory) => Shared = new(client, directory);
    ...
    public void Configure(Func<ReferenceDataOptions> getReferenceDataOptions)
    {
        ArgumentNullException.ThrowIfNull(getReferenceDataOptions);
        this.getReferenceDataOptions = getReferenceDataOptions;
    }
    ...
    public async Task<T> LoadAsync<T>(string url, Func<string, T> parse, CancellationToken cancellationToken = default)
    {
        ...
        var settings = getReferenceDataOptions();
